                ...
                <div class="space-y-4">
                    ...
                    <div v-if="product?.images?.length > 0" class="grid grid-cols-5 gap-4">
                        <button v-for="img in product.images" :key="img.id" 
                            ...
                            <img :src="getImagePath(img.image_path)" class="w-full h-full object-cover" />
                        </button>
                    </div>
                </div>
                        ...
                        <div v-if="product?.listing_type === 'auction'" class="space-y-8">
                            ...
                            <div v-if="product.listing_type === 'auction'">
                                ...
                                <div v-if="product.auction?.status === 'active' && new Date(product.auction?.end_time) > new Date()">
                                    ...
                                    <form @submit.prevent="placeBid" class="flex gap-4">
                                        <input 
                                        ...
                                        <button 
                                            ...
                                            class="bg-indigo-600 text-white px-10 py-5 rounded-2xl font-black hover:bg-gray-900 transition-all shadow-xl shadow-indigo-100 uppercase tracking-widest text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            <span v-if="bidForm.processing">İşleniyor...</span>
                    ...
                    <div v-for="related in relatedProducts" :key="related.id">
                        <Link :href="route('product.show', related.id)" class="group block bg-white rounded-[2.5rem] p-4 border border-gray-100 hover:shadow-2xl transition-all duration-500">
                            <div class="h-52 rounded-[2rem] overflow-hidden mb-6">
                                <img v-if="related.cover_image" :src="getImagePath(related.cover_image.image_path)" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-16">
                <div class="lg:col-span-1">
                    ...
                    <form @submit.prevent="submitComment" class="space-y-4">
                        <textarea v-model="commentForm.content" placeholder="Yorumunuzu yazın..." class="w-full bg-gray-50 border-gray-200 rounded-3xl p-5 text-sm focus:ring-2 focus:ring-indigo-600 min-h-[140px]" required></textarea>
                        <button type="submit" :disabled="commentForm.processing" class="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-sm hover:bg-black transition-all disabled:opacity-50">
                            Yorumu Gönder
                        </button>
                    </form>
                </div>
